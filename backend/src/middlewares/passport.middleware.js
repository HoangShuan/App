...
exports.jwtAuthentication = async (req, res, next) => {
  try {
    ...
    // DEBUG: log incoming token for troubleshooting auth issues
    console.log('[jwtAuth] incoming cookie token:', token ? '[REDACTED]' : null);
  ...
  const decoded = jwt.verify(token, process.env.JWT_SECRET_KEY);
    ...
    console.error('Authentication with JWT ERROR: ', error?.message || error);
...
passport.use(
  new GooglePlusTokenStrategy(
    ...
    async function (req, accessToken, refreshToken, profile, done) {
      ...
      console.log('[passport] Google auth attempt, profile:', profile?._json);
      try {
        if (!Boolean(profile)) {
          done(null, null);
          return;
